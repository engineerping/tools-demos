 ...
 * 循环 10 次后线程退出.
 */
class DelayThreadByRunnable implements Runnable{
    /**
     * 静态线程计数变量
     */
    ...
    public void run() {
        try {
            for (int i = 0; i < 10; i++) { //将循环体内的操作, 循环 10 次
                ...
                Thread.sleep(delay); //休眠 delay 毫秒
...
public class MyThreadSecond {
    ...
    public static void main(String[] args) {
        DelayThreadByRunnable thread1 = new DelayThreadByRunnable(); //DelayThreadByRunnable thread1
        DelayThreadByRunnable thread2 = new DelayThreadByRunnable(); //DelayThreadByRunnable thread2
        new Thread(thread1).start(); //通过有参构造器创建建线程对象, 并启动线程
        new Thread(thread2).start(); //通过有参构造器创建建线程对象, 并启动线程
        try{
